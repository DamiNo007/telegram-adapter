...
class GithubRequesterActor()(implicit val system: ActorSystem,
  ...
  def getGithubUser(username: String): Future[GithubUser] = {
    get(s"https://api.github.com/users/$username", Nil)
      .map { body =>
  ...
  def getUserRepositories(username: String): Future[List[GithubRepository]] = {
    get(s"https://api.github.com/users/${username}/repos", Nil)
      .map { body =>
  ...
  def mkListOfString(list: List[GithubRepository]): List[String] = {
    list.zipWithIndex.map {
      case (repoInfo, id) =>
        s"""${id + 1}. |${repoInfo.name}: size = ${repoInfo.size}, stargazers = ${repoInfo.stargazersCount}, push date = ${repoInfo.pushedAt}, fork = ${if (repoInfo.fork)
          "Forked"
        else "Not Forked"}"""
    }
  }
